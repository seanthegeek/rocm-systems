...
(see the `agent-handoff` skill) for goal, scope, and constraints.
...
Use the `agent-handoff` skill to build the dispatch — it is the single hand-off
...
Subagents speak the `agent-handoff` Expected Return shape. Read every return
