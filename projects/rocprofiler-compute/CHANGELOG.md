...
* Added native PC sampling collection. rocprof-compute captures stochastic and host-trap PC samples on every supporting GPU agent and writes them to ``ps_file_results.json``. The source files referenced by the sampled ISA are snapshotted into a ``code_obj_sources/`` directory alongside the results.
