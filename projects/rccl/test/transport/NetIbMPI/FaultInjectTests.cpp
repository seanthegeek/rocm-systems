...
//
// Ops-overload path. Unlike FaultInjCastQpErrorIsFatal (which uses the pre-call
// intercept ncclIbCastFaultSetQpError), this arms the REAL ibv_post_send via the
// shimmed ops table to return EAGAIN. The error surfaces through
// wrap_ibv_post_send → IbCastMultiSend → IbCastIsend, exercising the actual
// libibverbs call boundary.
//
// Verifies:
//   - isend returns an error OR fatalErrorCount > 0 when post_send is forced
//     to fail at the ops level.
// =============================================================================
...
//
// Ops-overload path, poll_cq STATUS-REWRITE mode. Requires NIC Fusion + failover.
// Arms the poll_cq shim to rewrite the status of completions on sender QP 0 to
// IBV_WC_WR_FLUSH_ERR — the whitelisted (recoverable) status. This reproduces a
// real CQE error at the verbs boundary; the resiliency layer should fail over to
// the surviving device and complete the transfer with no fatal error.
//
// Requires NCCL_IB_RESILIENCY_PORT_FAILOVER=1 and ndevs >= 2 (else SKIP).
// =============================================================================
...
//
// Ops-overload path, poll_cq SYNTHESIZE mode. Single device. Arms the poll_cq
// shim to fabricate an error completion with IBV_WC_REM_ACCESS_ERR (NOT in the
// failover whitelist, per ncclIbCastFaultCheckErrorFatal) when the CQ is idle.
// On a single-device topology there is no surviving device, so the injected
// fatal completion must surface as an error / fatal count.
//
// Verifies:
//   - isend fails OR fatalErrorCount > 0 (fatal, non-recoverable).
// =============================================================================
...
// FaultInjCastOpsPostSendErrno: arms the shimmed ibv_post_recv (via
// ncclIbCastFaultOpsSetPostRecvError) on every receiver QP to return EAGAIN.
// RCCL posts receive WRs for the CTS/data path through wrap_ibv_post_recv on
// the recv comm; forcing that to fail at the real ops boundary must surface as
// a recv-side error (PostRecv error or the transfer never completing).
//
// Exercises shimPostRecv's errno path in net_ib_ops_fault.cc, which no other
// test reaches (post_recv fault was previously uncovered).
//
// Verifies:
//   - With post_recv faulted on all receiver QPs, the receiver reports a
//     PostRecv error OR the recv never completes (the fault reached RCCL).
// =============================================================================
TEST_F(NetIbMPITest, FaultInjCastOpsPostRecvErrno) {
    ...
    if (rank == 0) {
        ...
        r0.sendRet = static_cast<int>(recvRet);  // reuse field: recv-side post result
        if (recvRet == ncclSuccess && recvReq != nullptr) {
            for (int poll = 0; poll < 200; poll++) {
                int done = 0, sz = 0;
                if (TestRequest(recvReq, &done, &sz) != ncclSuccess) {
                    r0.sendRet = static_cast<int>(ncclSystemError);
                    break;
                }
                if (done) { recvDone = true; break; }
                usleep(kPollIntervalUs);
    ...
    if (rank == 0) {
        ...
        bool recvErrored = (r0.sendRet != static_cast<int>(ncclSuccess));
        EXPECT_TRUE(recvErrored || !recvDone)
            << "rank 0: expected recv to error OR not complete after arming all "
            << r0.actualNqps << " receiver QPs with ops-level post_recv EAGAIN; "
            << "recv post/test returned " << r0.sendRet << ", recvDone=" << recvDone;
    }
...
//
// Unit-style coverage of the ops-overload fault API guard branches. Every
// public ncclIbCastFaultOps* entry validates its comm argument and the
// install/registration state before touching fault config. Those error guards
// (!ctx → ncclInvalidArgument; context not registered → ncclInvalidArgument /
// ncclSuccess for clear) are never hit by the functional tests, which always
// pass a live, shimmed connection.
//
// This test calls each API with a NULL comm and asserts ncclInvalidArgument,
// then on a live connection re-arms/clears to confirm the success guards too.
// No data is transferred; both ranks run identical checks (no inter-rank
// dependency), so assertions are safe on both ranks.
//
// Exercises the !ctx and registration-lookup guard branches in
// net_ib_ops_fault.cc (ncclIbOpsFaultArm*/Clear) plus the p2p.cc bridge
// validation.
// =============================================================================
