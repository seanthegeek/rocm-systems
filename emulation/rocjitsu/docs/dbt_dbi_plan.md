...
| `AmdGpuCodeObject` (`code/amdgpu_code_object.h`) | ELF parser; `kernel_descriptor_offset()` looks up `.kd` symbols; `text_sections()` returns DBT-visible executable text; `image_data()` / `image_size()` give the raw ELF bytes to the patcher |
